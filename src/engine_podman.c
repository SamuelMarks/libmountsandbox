...
static int podman_execute(const sandbox_config_t *config, int argc,
  ...
  if (config->denied_syscalls && config->denied_syscall_count > 0) {
    size_t c;
    fprintf(stderr, "[libmountsandbox] Warning: Translating Podman syscall "
 ...
 * \param process The process handle.
 */
static void podman_free_process(sandbox_process_t *process) { (void)process; }
 ...
 */
sandbox_engine_t engine_podman = {"podman",
