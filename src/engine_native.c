...
static int native_execute(const sandbox_config_t *config, int argc, char **argv) {
    ...
    const char *profile_template_base = "(version 1)\n"
                                        "(allow default)\n"
                                        "(deny file-write*)\n"
    ...
    if (config->use_pty) {
        fprintf(stderr, "[libmountsandbox] Warning: Interactive PTY is not natively supported on Windows Job Objects without ConPTY. Ignored.\n");
    }
    ...
    if (config->seccomp_profile_path || config->apparmor_profile) {
        fprintf(stderr, "[libmountsandbox] Warning: Seccomp/AppArmor profiles are not natively supported on macOS. Ignored.\n");
    }
    if (config->seccomp_profile_path || config->apparmor_profile) {
        fprintf(stderr, "[libmountsandbox] Warning: Seccomp/AppArmor profiles are not natively supported via Windows Job Objects. Ignored.\n");
    }
    if (config->denied_syscalls && config->denied_syscall_count > 0) {
        size_t c;
        for (c = 0; c < config->denied_syscall_count; c++) {
            /* (deny system-mac-syscall (mac-syscall-number 0)) is seatbelt but we use simple deny process-exec or similar approximate mappings since true syscall filtering in Seatbelt requires precise private API macros. We emit a warning. */
        }
        fprintf(stderr, "[libmountsandbox] Warning: Fine-grained syscall filtering is not supported natively via macOS Seatbelt. Ignored.\n");
    }
    ...
    if (pid < 0) {
        ...
        if (config->apparmor_profile) {
            ...
            if (afp) {
                ...
                fprintf(stderr, "[libmountsandbox] Warning: Could not open /proc/self/attr/exec to set AppArmor profile\n");
            }
        }
        ...
        if (config->use_pty && master_fd >= 0) {
            ...
            setsid();
#if defined(__APPLE__)
            /* macOS setsid creates a new session but TIOCSCTTY is not standard, login_tty is better but open handles it on macOS if O_NOCTTY is not used */
#endif
        ...
        if (config->max_cpu_percent > 0) {
            fprintf(stderr, "[libmountsandbox] Warning: CPU rate limiting not natively supported on macOS without Docker.\n");
        }
        ...
        /* Parent process */
        int wstatus;
        unsigned int max_polls = config->timeout_secs > 0 ? config->timeout_secs * 10 : 0;
            ...
            if (wp != 0) {
                if (config->use_pty && master_fd >= 0) {
                    ...
                    while ((n = read(master_fd, buf, sizeof(buf))) > 0) {
                        if (out_fp) fwrite(buf, 1, n, out_fp);
                    }
                }
...
static int native_execute(const sandbox_config_t *config, int argc, char **argv) {
    ...
    if (config->denied_syscalls && config->denied_syscall_count > 0) {
        base_args += (int)(config->denied_syscall_count * 2); /* --cap-drop SYSCALL */
    }

    if (config->mounts && config->mount_count > 0) {
        base_args += (int)(config->mount_count * 3); /* --bind / --ro-bind, SRC, DEST */
    }
    ...
    if (config->denied_syscalls && config->denied_syscall_count > 0) {
        size_t c;
        fprintf(stderr, "[libmountsandbox] Warning: Translating Linux seccomp deny into capabilities drop for bwrap.\n");
        for (c = 0; c < config->denied_syscall_count; c++) {
    ...
    if (pid < 0) {
        ...
        if (config->max_cpu_percent > 0) {
            fprintf(stderr, "[libmountsandbox] Warning: CPU rate limiting not natively supported via bwrap without Cgroups/Docker.\n");
        }
    ...
    if (config->drop_privileges) {
        ...
        for (j = 0; j < current_arg; j++) {
            if (exec_argv[j] && j > 0) {
                if (strcmp(exec_argv[j-1], "--uid") == 0 || strcmp(exec_argv[j-1], "--gid") == 0) {
                    free(exec_argv[j]);
                }
            }
        }
...
static int native_execute(const sandbox_config_t *config, int argc, char **argv) {
    ...
    if (config->max_cpu_percent > 0) {
        fprintf(stderr, "[libmountsandbox] Warning: CPU rate limiting not enabled on baseline Windows Job Objects without newer flags.\n");
    }

    if (config->denied_syscalls && config->denied_syscall_count > 0) {
        fprintf(stderr, "[libmountsandbox] Warning: Fine-grained Syscall filtering (Seccomp) is not natively supported via Windows Job Objects. Ignored.\n");
    }
    ...
    if (config->disable_network) {
        /* Note: Network disabling natively on Windows requires Windows Filtering Platform (WFP), 
         * AppContainers, or Firewall rules, which is highly complex in raw C89.
         * For this baseline, we warn the user. */
        fprintf(stderr, "[libmountsandbox] Warning: Network disabling not natively supported in Windows Job Objects yet.\n");
    }
    ...
    if (out_fp || err_fp) {
        ...
        if (out_fp) si.hStdOutput = (HANDLE)_get_osfhandle(_fileno(out_fp));
        else si.hStdOutput = GetStdHandle(STD_OUTPUT_HANDLE);
        
        if (err_fp) si.hStdError = (HANDLE)_get_osfhandle(_fileno(err_fp));
...
static int native_execute(const sandbox_config_t *config, int argc, char **argv) {
    ...
    fprintf(stderr, "[libmountsandbox] Native sandbox not supported on this platform.\n");
    return -1;
}
...
static sandbox_process_t* native_execute_async(const sandbox_config_t *config, int argc, char **argv) {
    (void)config; (void)argc; (void)argv;
    fprintf(stderr, "[libmountsandbox] Async execution is a work in progress for native. Falling back to sync.\n");
    return NULL;
}
...
static void native_free_process(sandbox_process_t *process) {
    (void)process;
}
