 ...
 * It maps libmountsandbox configuration intents into Wasmtime WASI capabilities.
 * It strictly supports C89 and compiles across multiple compilers by using
 * platform-specific process execution semantics.
 */
...
static int wasmtime_execute(const sandbox_config_t *config, int argc, char **argv) {
    char **wasmtime_argv;
    int i, status = -1;
    int current_arg = 0;
    ...
    if (!config || !argv || argc < 1) return -1;
    ...
    if (config->seccomp_profile_path || config->apparmor_profile || config->denied_syscall_count > 0) {
        fprintf(stderr, "[libmountsandbox] Warning: OS-level Syscall filtering ignored. WebAssembly relies on WASI capabilities natively.\n");
    }

    if (!config->disable_network) {
        fprintf(stderr, "[libmountsandbox] Warning: Network access is inherently disabled in default WASI unless explicit socket capabilities are mapped.\n");
    }

    if (config->max_memory_mb > 0 || config->max_cpu_percent > 0) {
        fprintf(stderr, "[libmountsandbox] Warning: CPU/Memory rate limiting via CLI is not mapped in this implementation of Wasmtime.\n");
    }

    if (config->drop_privileges) {
        fprintf(stderr, "[libmountsandbox] Warning: Privilege dropping ignored. WASM executes securely in userspace.\n");
    }

    if (config->use_pty) {
        fprintf(stderr, "[libmountsandbox] Warning: Wasmtime engine ignores use_pty as interactive TTY routing requires complex WASI fd mapping.\n");
    }

    wasmtime_argv = (char **)malloc((size_t)(argc + base_args + 1) * sizeof(char *));
    if (!wasmtime_argv) {
...
#if defined(_WIN32) || defined(__WATCOMC__)
    {
        ...
        status = _spawnvp(_P_WAIT, "wasmtime", (const char * const *)wasmtime_argv);
...
static sandbox_process_t* wasmtime_execute_async(const sandbox_config_t *config, int argc, char **argv) {
    (void)config; (void)argc; (void)argv;
    fprintf(stderr, "[libmountsandbox] Async execution is a work in progress for wasmtime. Falling back to sync.\n");
    return NULL;
}
...
static void wasmtime_free_process(sandbox_process_t *process) {
    (void)process;
}
