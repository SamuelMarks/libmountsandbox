/**
 * \file engine_docker.c
 * \brief Docker-based sandbox implementation.
 *
 ...
 */
static int docker_execute(const sandbox_config_t *config, int argc, char **argv) {
    char **docker_argv;
    int i;
    int status = -1;
    ...
    docker_argv = (char**)malloc((size_t)(argc + base_args + 1) * sizeof(char*));
    if (!docker_argv) {
        return -1;
    }

    docker_argv[current_arg++] = (char*)"docker";
    docker_argv[current_arg++] = (char*)"run";
    docker_argv[current_arg++] = (char*)"--rm";
    
    if (config->disable_network) {
        docker_argv[current_arg++] = (char*)"--network";
        docker_argv[current_arg++] = (char*)"none";
    }
    if (config->use_pty) {
        docker_argv[current_arg++] = (char*)"-i";
        docker_argv[current_arg++] = (char*)"-t";
    }
    if (config->seccomp_profile_path) {
        sec_buf_ptr = (char*)malloc(strlen(config->seccomp_profile_path) + 15);
        if (sec_buf_ptr) {
            sprintf(sec_buf_ptr, "seccomp=%s", config->seccomp_profile_path);
            docker_argv[current_arg++] = (char*)"--security-opt";
            docker_argv[current_arg++] = sec_buf_ptr;
        }
    }
    if (config->apparmor_profile) {
    ...
    if (config->drop_privileges) {
        ...
        docker_argv[current_arg++] = (char*)"-u";
        docker_argv[current_arg++] = user_buf;
    }

    if (config->denied_syscalls && config->denied_syscall_count > 0) {
        size_t c;
        fprintf(stderr, "[libmountsandbox] Warning: Translating Docker syscall deny into capabilities drop (approximate).\n");
        for (c = 0; c < config->denied_syscall_count; c++) {
            docker_argv[current_arg++] = (char*)"--cap-drop";
            docker_argv[current_arg++] = (char*)config->denied_syscalls[c];
        }
    }
    ...
    if (config->env_vars && config->env_count > 0) {
        ...
        for (e = 0; e < config->env_count; e++) {
            docker_argv[current_arg++] = (char*)"-e";
            docker_argv[current_arg++] = (char*)config->env_vars[e];
        }
    }
    ...
    free(docker_argv);
            ...
            vstr_array[m] = vstr;
            docker_argv[current_arg++] = (char*)"-v";
            docker_argv[current_arg++] = vstr;
        }
    }

    /* Set working directory to the first mount point, or fallback */
    docker_argv[current_arg++] = (char*)"-w";
    if (config->mount_count > 0) {
        docker_argv[current_arg++] = (char*)"/workspace0";
    } else {
        docker_argv[current_arg++] = (char*)"/";
    }
    docker_argv[current_arg++] = (char*)"ubuntu:latest";

    for (i = 0; i < argc; i++) {
        docker_argv[current_arg++] = argv[i];
    }
    docker_argv[current_arg] = NULL;
...
#if defined(_WIN32) || defined(__WATCOMC__)
    {
        ...
        if (config->timeout_secs > 0) {
#ifdef _WIN32
            intptr_t hProcess = _spawnvp(_P_NOWAIT, "docker", (const char* const*)docker_argv);
            if (hProcess != -1) {
            ...
            status = (int)_spawnvp(_P_WAIT, "docker", (const char* const*)docker_argv);
#endif
        } else {
            status = (int)_spawnvp(_P_WAIT, "docker", (const char* const*)docker_argv);
        }
...
static sandbox_process_t* docker_execute_async(const sandbox_config_t *config, int argc, char **argv) {
    (void)config; (void)argc; (void)argv;
    fprintf(stderr, "[libmountsandbox] Async execution is a work in progress for docker. Falling back to sync.\n");
    return NULL;
}

static int docker_wait_process(sandbox_process_t *process, int *exit_status) {
    (void)process; (void)exit_status;
    return -1;
}

static void docker_free_process(sandbox_process_t *process) {
    (void)process;
}

static void docker_cleanup(void) {
 ...
 */
sandbox_engine_t engine_docker = {
    "docker",
    "Docker-based secure container sandbox",
    docker_init,
    docker_execute,
    docker_execute_async,
    docker_wait_process,
    docker_free_process,
    docker_cleanup
};
