...
static int gvisor_execute(const sandbox_config_t *config, int argc, char **argv) {
    ...
    gvisor_argv = (char**)malloc((size_t)(argc + base_args + 1) * sizeof(char*));
    if (!gvisor_argv) {
        return -1;
    }
...
#if defined(_WIN32) || defined(__WATCOMC__)
    {
        ...
        if (config->timeout_secs > 0) {
#ifdef _WIN32
            intptr_t hProcess = _spawnvp(_P_NOWAIT, "docker", (const char* const*)gvisor_argv);
            if (hProcess != -1) {
            ...
            status = (int)_spawnvp(_P_WAIT, "docker", (const char* const*)gvisor_argv);
#endif
        } else {
            status = (int)_spawnvp(_P_WAIT, "docker", (const char* const*)gvisor_argv);
        }
...
static sandbox_process_t* gvisor_execute_async(const sandbox_config_t *config, int argc, char **argv) {
    (void)config; (void)argc; (void)argv;
    fprintf(stderr, "[libmountsandbox] Async execution is a work in progress for gvisor. Falling back to sync.\n");
    return NULL;
}
...
static void gvisor_free_process(sandbox_process_t *process) {
    (void)process;
}
