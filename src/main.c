...
void print_version(void) {
    printf("libmountsandbox version 0.1.0\n");
}
...
void print_usage(const char *prog_name) {
    ...
    fprintf(stderr, "  --engine=<type>         Select sandbox engine (docker, dummy, native [default])\n");
    ...
    fprintf(stderr, "  --timeout=SECS          Set execution timeout in seconds\n");
    ...
    fprintf(stderr, "  --uid=UID               Set target UID (implies --drop-privs)\n");
    fprintf(stderr, "  --gid=GID               Set target GID (implies --drop-privs)\n");
    fprintf(stderr, "  --deny-syscall=SYSCALL  Deny specific syscall\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Use '--' before <command> to ensure subsequent arguments are treated as command arguments rather than sandbox options.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Example: %s --engine=native --drop-privs --deny-syscall=ptrace --mount=/tmp -- curl http://google.com\n", prog_name);
}
...
static int split_and_append(const char *str, char sep, const char **array, size_t *count) {
    char *copy = NULL;
    char *p, *start;
    if (!str) return 0;
    if (my_strdup(str, &copy) != 0) return -1;
    start = copy;
...
static int split_and_append_mount(const char *str, char sep, int read_only, sandbox_mount_t *array, size_t *count) {
    char *copy = NULL;
    char *p, *start;
    if (!str) return 0;
    if (my_strdup(str, &copy) != 0) return -1;
...
int main(int argc, char **argv) {
    ...
    const char *env_engine = getenv("MOUNTSANDBOX_ENGINE");
    const char *env_no_network = getenv("MOUNTSANDBOX_NO_NETWORK");
    const char *env_env = getenv("MOUNTSANDBOX_ENV");
    const char *env_mount = getenv("MOUNTSANDBOX_MOUNT");
    const char *env_ro_mount = getenv("MOUNTSANDBOX_RO_MOUNT");
    const char *env_timeout = getenv("MOUNTSANDBOX_TIMEOUT");
    const char *env_memory_mb = getenv("MOUNTSANDBOX_MEMORY_MB");
    const char *env_cpu_pct = getenv("MOUNTSANDBOX_CPU_PCT");
    const char *env_drop_privs = getenv("MOUNTSANDBOX_DROP_PRIVS");
    const char *env_uid = getenv("MOUNTSANDBOX_UID");
    const char *env_gid = getenv("MOUNTSANDBOX_GID");
    const char *env_deny_syscall = getenv("MOUNTSANDBOX_DENY_SYSCALL");

    int extra_envs = env_env ? count_char(env_env, ',') + 1 : 0;
    int extra_mounts = (env_mount ? count_char(env_mount, ',') + 1 : 0) + 
                       (env_ro_mount ? count_char(env_ro_mount, ',') + 1 : 0);
    int extra_syscalls = env_deny_syscall ? count_char(env_deny_syscall, ',') + 1 : 0;
    ...
    env_vars_buf = (const char **)malloc((size_t)(argc + extra_envs) * sizeof(const char *));
    mounts_buf = (sandbox_mount_t *)malloc((size_t)(argc + extra_mounts + 1) * sizeof(sandbox_mount_t));
    syscalls_buf = (const char **)malloc((size_t)(argc + extra_syscalls) * sizeof(const char *));
    if (!env_vars_buf || !mounts_buf || !syscalls_buf) {
        fprintf(stderr, "Error: out of memory\n");
        if (env_vars_buf) free((void*)env_vars_buf);
        if (mounts_buf) free((void*)mounts_buf);
        if (syscalls_buf) free((void*)syscalls_buf);
        return EXIT_FAILURE;
    }
    ...
    if (env_no_network && strcmp(env_no_network, "0") != 0 && strcmp(env_no_network, "false") != 0) {
    ...
    if (env_drop_privs && strcmp(env_drop_privs, "0") != 0 && strcmp(env_drop_privs, "false") != 0) {
    ...
    if (env_env && split_and_append(env_env, ',', env_vars_buf, &env_count) != 0) return EXIT_FAILURE;
    if (env_mount && split_and_append_mount(env_mount, ',', 0, mounts_buf, &mount_count) != 0) return EXIT_FAILURE;
    if (env_ro_mount && split_and_append_mount(env_ro_mount, ',', 1, mounts_buf, &mount_count) != 0) return EXIT_FAILURE;
    if (env_deny_syscall && split_and_append(env_deny_syscall, ',', syscalls_buf, &syscall_count) != 0) return EXIT_FAILURE;
    ...
    for (i = 0; i < (int)config.mount_count; i++) {
        printf("[libmountsandbox] Mount: %s (%s)\n", config.mounts[i].dir, config.mounts[i].read_only ? "ro" : "rw");
    }
